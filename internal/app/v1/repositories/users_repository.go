...
func (repository *UserRepositoryImpl) FindByID(id int) (*domain.Users, error) {
	...
	result := repository.db.Preload("Credential").Preload("ArticleFavorites").Preload("ArticleFavorites.Admin").Preload("ArticleFavorites.Counselors").Where("id = ?", id).First(&user)
	if result.Error != nil {
		return nil, result.Error
	}
